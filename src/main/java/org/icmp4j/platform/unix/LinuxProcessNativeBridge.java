...
/**
 * Internet Control Message Protocol for Java (ICMP4J) http://www.icmp4j.org
 ...
 * http://www.icmp4j.org 5. You do all of the above within 4 weeks of
 ...
 * <p/>
 * User: Sal Ingrilli Date: May 23, 2014 Time: 9:38:18 PM
 */
public class LinuxProcessNativeBridge extends NativeBridge {
     ...
     *
     * Executes the given icmp ECHO request This call blocks until a response is
    ...
    public IcmpPingResponse executePingRequest(final IcmpPingRequest request) {
        ...
        logger.debug("IcmpPingRequest details: " + request.toString());
        // handle exceptions
        try {
            ...
            logger.debug("icmpSendEchoDuration: " + icmpSendEchoDuration);
            // check for timeout
            final boolean timeoutFlag = icmpSendEchoDuration >= timeout;
            logger.debug("timeoutFlag is " + timeoutFlag);
            if (timeoutFlag) {
            ...
            logger.debug("Returning executePingRequest (stringList)");
    ...
    public IcmpPingResponse executePingRequest(final List<String> stringList) {
        ...
        for (final String string : stringList) {
            ...
            logger.debug("icmpReqIndex: " + icmpReqIndex);
            final int icmpSeqIndex = string.indexOf("icmp_seq=");
            logger.debug("icmpSeqIndex: " + icmpSeqIndex);
            ...
            int size = 0;
            {
                ...
                logger.debug("bytesIndex: " + bytesIndex);
                if (bytesIndex > 0) {
                    final String sizeAsString = string.substring(0, bytesIndex);
                    logger.debug("About to parseInt sizeAsString, which is: " + sizeAsString);
